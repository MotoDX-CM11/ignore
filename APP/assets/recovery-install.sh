...
if [ -d $INSTALLPATH/install-files ]; then
	su -c "$BBX rm -rf $INSTALLPATH/install-files $LOGFILE"
fi
sync
su -c "$BBX unzip $INSTALLPATH/install-files.zip -d $INSTALLPATH $LOGFILE"
if [ ! -d $INSTALLPATH/install-files ]; then
	echo 'ERR: Zip file didnt extract correctly.  Installation aborted.' $LOGFILE
	exit 1
fi
su -c "$BBX chmod -R 755 $INSTALLPATH/install-files"
